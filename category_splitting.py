...
df = pd.read_csv('/Users/yasharya/Projects/Opinio/FitSight-Produhacks2024/DATA/sentiment_reviews_withcount copy.csv')
...
def get_combined_categories(ldamodel, num_topics, num_keywords=5):
    ...
    most_common_keywords = prioritized + [word for word, count in Counter(filtered_keywords).most_common(num_keywords) if word not in prioritized]
    
    return most_common_keywords[:num_keywords]  # Limit to the requested number of keywords
